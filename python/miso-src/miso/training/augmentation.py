...
def _rotate_image(image, angle, interpolation='bilinear'):
    """Rotate an image by the given angle in radians.
    Replacement for tfa.image.rotate (tensorflow_addons is deprecated).
    """
    cos_angle = tf.math.cos(angle)
    sin_angle = tf.math.sin(angle)
    # Build the 2D rotation matrix as a flat [8] vector for tf.raw_ops.ImageProjectiveTransformV3
    h = tf.cast(tf.shape(image)[0], tf.float32)
    w = tf.cast(tf.shape(image)[1], tf.float32)
    cx = w / 2.0
    cy = h / 2.0
    # Transform matrix maps output coords to input coords
    a0 = cos_angle
    a1 = sin_angle
    a2 = cx - cos_angle * cx - sin_angle * cy
    b0 = -sin_angle
    b1 = cos_angle
    b2 = cy + sin_angle * cx - cos_angle * cy
    transforms = tf.reshape(tf.stack([a0, a1, a2, b0, b1, b2, 0.0, 0.0]), [1, 8])
...
def aug_rotation(im_x, rotation=(0, 360)):
    if rotation is not None:
        ...
        im_x = _rotate_image(im_x, rotation_factor, interpolation='bilinear')
    return im_x
